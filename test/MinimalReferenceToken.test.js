...
const OldMinimalReferenceToken = artifacts.require('MinimalReferenceToken');
const utils = require('./utils');

contract('MinimalReferenceToken', function(accounts) {
  const provider = new URL(this.web3.currentProvider.host);
  provider.protocol = 'ws';
  const web3 = new Web3(provider.toString());
  const MinimalReferenceToken = new web3.eth.Contract(
    OldMinimalReferenceToken.abi,
    { data: OldMinimalReferenceToken.bytecode }
  );

  let token = {
    name: 'MinimalReferenceToken',
    symbol: 'MRT',
    granularity: '0.01',
    totalSupply: '0',
    defaultBalance: '0',
  ...
  const deployContract = MinimalReferenceToken
    .deploy({ arguments: [
      token.name,
      token.symbol,
